...
def show_video_info(data):
    '''
    Muestra una ventana con la info del vídeo.
    
    Si se pasa un item como argumento usa el scrapper Tmdb para buscar la info del vídeo
      ...
      En caso de series:
        1. Busca la temporada y eìsodio en los campos contentSeason y contentepisodeNumber
      ...
      Si se pasa como argumento un dict() muestra en la ventana directamente la información pasada (sin usar el scrapper)
        Formato:
          En caso de peliculas:
            dict({ 
                     "type"           : "movie",
    ...
    return InfoWindow("InfoWindow.xml", config.get_runtime_path()).Start(data)